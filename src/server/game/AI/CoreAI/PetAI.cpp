...
void PetAI::UpdateAI(uint32 diff)
{
	...
	//  charm info must be always available
	if (!me->GetCharmInfo())
		return;
    ...
    if (!me->HasUnitState(UNIT_STATE_CASTING))
    {
		if (owner && owner->GetTypeId() == TYPEID_PLAYER && me->GetCharmInfo()->GetForcedSpell() && me->GetCharmInfo()->GetForcedTarget())
		{
			...
			//  if spell was casted properly and we are in passive mode, handle return
		...
		//  dont allow ghouls to cast spells below 75 energy
        ...
        for (uint8 i = 0; i < me->GetPetAutoSpellSize(); ++i)
        {
            ...
            if (spellInfo->IsPositive())
            {
				...
				spell->LoadScripts(); //  load for CanAutoCast (calling CheckPetCast)
...
void PetAI::KilledUnit(Unit* victim)
{
	...
	//  if pet is channeling a spell and owner killed something different, dont interrupt it
...
void PetAI::HandleReturnMovement()
{
	...
	//  remember that npcs summoned by npcs can also be pets
	me->DeleteThreatList();
	me->ClearInPetCombat();
}
...
bool PetAI::CanAttack(Unit* target, const SpellInfo* spellInfo)
{
	...
	//  check unit states of pet
	if (me->HasUnitState(UNIT_STATE_LOST_CONTROL))
		return false;

	//  pets of mounted players have stunned flag only, check this also
