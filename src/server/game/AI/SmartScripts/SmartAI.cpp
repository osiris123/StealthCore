...
SmartAI::SmartAI(Creature* c) : CreatureAI(c)
{
    ...
    // spawn in run mode
	//  spawn in run mode and set mRun to run... this overrides SetWalk EVERYWHERE, RETARDS
    mRun = true;
...
void SmartAI::GenerateWayPointArray(Movement::PointsArray* points)
{
    ...
    {
        //  first point in vector is unit real position
        points->clear();
...
void SmartAI::EndPath(bool fail)
{
	...
	//  if the escort failed - DO NOT PROCESS ANYTHING, ITS RETARDED
...
void SmartAI::UpdatePath(const uint32 diff)
{
	...
	if (!me->HasUnitState(UNIT_STATE_NOT_MOVE) && me->movespline->Finalized())//reached WP
    {
        ...
        {
            SetRun(mRun);
            //  if we have reached waypoint, and there is no working spline movement it means our splitted array has ended, make new one
...
{
	//  fixes strange jumps when charming SmartAI npc
    ...
    else if (mFollowGuid)
    {
		...
		//  do not forget to reset scripts as we wont call reached home
		GetScript()->OnReset();
    }
    else
	{
...
{
	//  if unit has no victim, or victim is player controlled thing
    ...
    //  victim of unit has to be a player controlled unit
    if (!IS_PLAYER_GUID(who->GetVictim()->GetCharmerOrOwnerOrOwnGUID()))
        return false;

    //  Check if victim can be assisted
...
void SmartAI::DamageTaken(Unit* doneBy, uint32& damage, DamageEffectType damagetype, SpellSchoolMask /*damageSchoolMask*/)
{
	...
	//  skip nodamage type (eg. instakill effect)
...
void SmartAI::CorpseRemoved(uint32& respawnDelay)
{
	...
	//  end escort upon corpse remove, safe check in case of immediate despawn
	if (IsEscorted())
		EndPath(true);
}
...
{
	//  Fix Combat Movement
	if (GetScript()->GetMaxCombatDist()/* || GetScript()->GetCasterMaxDist()*/) //  we only need this hack for old caster movement system
		return;
...
{
	//  i dont think this is necessery (can be made as event parameter)
    //if (!result)
    //    return;
