...
void SmartScript::ProcessAction(SmartScriptHolder& e, Unit* unit, uint32 var0, uint32 var1, bool bvar, const SpellInfo* spell, GameObject* gob)
{
    ...
    switch (e.GetActionType())
    {
        case SMART_ACTION_TALK:
        {
			...
			Creature* talker = e.target.type == 0 ? me : NULL; //  tc retardness fix
        ...
        {
			//  do not allow to flee without control (stun, fear etc)
        ...
        case SMART_ACTION_SET_FLY:
        {
			...
			//  this wil be executed only if state is different
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
            {
				if (IsCreature(*itr))
				{
					if (IsSmart((*itr)->ToCreature()))
						...
						(*itr)->ToCreature()->SetWalk(e.action.setRun.run ? false : true); //  reversed
				}
            }
        ...
        case SMART_ACTION_MOVE_TO_POS:
        {
            ...
            /*if (e.GetTargetType() == SMART_TARGET_CREATURE_RANGE || e.GetTargetType() == SMART_TARGET_CREATURE_GUID ||
				...
				e.GetTargetType() == SMART_TARGET_SELF || e.GetTargetType() == SMART_TARGET_STORED) //  bieda i rozpierdol TC)*/
            {
                if (ObjectList* targets = GetTargets(e, unit))
				{
        ...
        case SMART_ACTION_RESPAWN_TARGET:
        {
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
            {
				...
				{
					//  do not modify respawndelay of already spawned gameobjects QQ
...
ObjectList* SmartScript::GetTargets(SmartScriptHolder const& e, Unit* invoker /*= NULL*/)
{
    ...
    switch (e.GetTargetType())
    {
        ...
        case SMART_TARGET_STORED:
        {
			...
			//  return l, retardness... what if list is empty? will return empty list instead of null pointer
            break;
        }
...
void SmartScript::ProcessEvent(SmartScriptHolder& e, Unit* unit, uint32 var0, uint32 var1, bool bvar, const SpellInfo* spell, GameObject* gob)
{
    ...
    switch (e.GetEventType())
    {
        ...
        case SMART_EVENT_ON_SPELLCLICK:
            ProcessAction(e, unit, var0, var1, bvar, spell, gob);
            break;
		//  added no report use distinction for gameobjects
...
void SmartScript::InitTimer(SmartScriptHolder& e)
{
    switch (e.GetEventType())
    {
        ...
        case SMART_EVENT_OOC_LOS:
        case SMART_EVENT_IC_LOS:
            //  wtf is this bullshit? cooldown should be processed AFTER action is done, not before...
            //RecalcTimer(e, e.event.los.cooldownMin, e.event.los.cooldownMax);
            //break;
        case SMART_EVENT_DISTANCE_CREATURE:
        case SMART_EVENT_DISTANCE_GAMEOBJECT:
...
void SmartScript::OnInitialize(WorldObject* obj, AreaTriggerEntry const* at)
{
    ...
    for (SmartAIEventList::iterator i = mEvents.begin(); i != mEvents.end(); ++i)
	{
		...
		//  if smartcast combat move flag is present
