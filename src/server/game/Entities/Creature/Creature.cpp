...
{ 
	...
	//  NPC is in combat, keep this flag!
...
{ 
	...
	//  implement power regeneration flag
...
{ 
    ...
    //  Initialize vehicle if it is not summoned!
...
{ 
	...
	else if (cinfo->type_flags & CREATURE_TYPEFLAGS_GHOST) //  Add ghost visibility for ghost units
...
{ 
    ...
    if (!assist)
        if (IsNeutralToAll() || !IsWithinDistInMap(who, GetAggroRange(who) + m_CombatDistance))
            return false;
...
{ 
    ...
    if (s == JUST_DIED)
    {
        ...
        if (!despawn && (CanFly() || IsFlying()) && !HasUnitMovementFlag(MOVEMENTFLAG_SWIMMING))
    ...
    else if (s == JUST_RESPAWNED)
    {
        ...
        //  Load auras AFTER setting alive state
...
bool Creature::IsImmunedToSpell(SpellInfo const* spellInfo)
{
	...
	//  this should exclude self casts...
...
{ 
	//  pets, guardians and units with scripts / smartAI should be skipped
...
{ 
    ...
    if (!IS_PLAYER_GUID(GetCharmerOrOwnerGUID()))
	{
		...
		//  this should include taunt auras
	...
	//  added size factor for huge npcs
