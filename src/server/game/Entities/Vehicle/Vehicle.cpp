...
bool Vehicle::AddPassenger(Unit* unit, int8 seatId)
{
	...
	//  moved from unit.cpp, if aura passes seatId == -1 (choose automaticly) we wont get appropriate flags
	...
	//  removed retarded seat->first == 0 check...
	if (_me->GetTypeId() == TYPEID_UNIT
    ...
    if (_me->IsInWorld())
    {
		...
		//  did not found anything unique in dbc, maybe missed something
