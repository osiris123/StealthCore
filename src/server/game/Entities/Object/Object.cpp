...
{ 
    ...
    if (distanceCheck)
    {
        ...
        if (Player const* thisPlayer = ToPlayer())
        {
			...
			{
				//  don't allow to detect vehicle accessory if you can't see vehicle base!
		...
		//  check reversely obj vs viewpoint, object could be a gameObject which overrides _IsWithinDist function to include gameobject size
...
{ 
    ...
    if (obj->IsAlwaysDetectableFor(seer) || GetEntry() == WORLD_TRIGGER) //  World Trigger can detect all objects, used for wild gameobjects without owner!
        return true;

    if (!ignoreStealth)
    {
        if (!seer->CanDetectInvisibilityOf(obj)) //  added ignoreStealth, allow AoE spells to hit invisible targets!
            return false;

        if (!seer->CanDetectStealthOf(obj))
        {
            //  ignore units players have at client, this cant be cheated!
            if (checkClient)
            {
...
{ 
	...
	//  include invisible flags of caster in the mask, 2 invisible objects should be able to detect eachother
...
{ 
	...
	//  if gameobject is temporary, set custom spellid
	if (respawnTime)
		go->SetSpellId(1);
...
{ 
	...
	//  correctly set phase mask in case of gameobjects
...
{ 
	...
	//  check if last z updates did not move z too far away
...
{ 
	...
	//  if we have gameobject, store last valid ground position
	//  I assume you wanted to spawn totem in air and allow it to fall down if no valid position was found
	if (forGameObject)
		prevdz = destz;
