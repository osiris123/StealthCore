...
{ 
    ...
    switch (m_deathState)
    {
        ...
        case ALIVE:
        {
            ...
            // : m_regenTimer is decrased in Creature::Update()
			// : just check if we can update focus in current period
...
{ 
    ...
    // Resistance
	// : hunter pets should not inherit template resistances
	if (!IsHunterPet())
    ...
    else                                            // not exist in DB, use some default fake data
    {
		...
		// : multiply base values by creature_template factors!
		...
		// : added some multipliers so debuffs can affect pets in any way...
        ...
        case SUMMON_PET:
        {
			...
			switch(GetEntry())
            {
                case NPC_FELGUARD:
                {
					...
					// : Glyph of Felguard, so ugly im crying... no appropriate spell
	...
	{
		// : fixes orc death knight command racial
...
{ 
	...
	else// if (GetAsynchLoadType() == PET_LOAD_BG_RESURRECT || GetAsynchLoadType() == PET_LOAD_SUMMON_PET || GetAsynchLoadType() == PET_LOAD_SUMMON_DEAD_PET)
	{
		...
		{
			setDeathState(ALIVE);
			ClearUnitState(uint32(UNIT_STATE_ALL_STATE & ~(UNIT_STATE_POSSESSED))); // : added just in case... some linked auras and so on
			SetHealth(CountPctFromMaxHealth(15)); // : well, only two pet reviving spells exist and both revive with 15%
			SavePetToDB(PET_SAVE_AS_CURRENT, false);
		}
	}

	// : resurrect with full health if resurrected by BG / BF spirit
	if (GetAsynchLoadType() == PET_LOAD_BG_RESURRECT)
		SetHealth(GetMaxHealth());

	// : We are summoned in arena / battleground, remove all positive auras
	// : and set health to full if in preparation phase
