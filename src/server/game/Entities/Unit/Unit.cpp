...
uint32 Unit::DealDamage(Unit* attacker, Unit* victim, uint32 damage, CleanDamage const* cleanDamage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellInfo const* spellProto, bool durabilityLoss, bool allowGM)
{
    ...
    if (attacker && attacker != victim)
		...
		{
			// don't allow GMs to deal damage in normal way (this leaves no evidence in logs!), they have commands to do so
...
{ 
    ...
    else
    {
        ...
        {
			float dodge = ToCreature()->isWorldBoss() ? 5.85f : 5.0f;
        ...
        for (SpellImmuneList::const_iterator itr = list.begin(); itr != list.end(); ++itr)
            if (itr->type == aura && (itr->spellId != 64848 || spellInfo->Effects[index].MiscValue == POWER_MANA))
                if (!spellInfo->HasAttribute(SPELL_ATTR3_IGNORE_HIT_RESULT))
					if (itr->blockType == SPELL_BLOCK_TYPE_ALL || spellInfo->IsPositive())
						return true;

        // Check for immune to application of harmful magical effects
        AuraEffectList const& immuneAuraApply = GetAuraEffectsByType(SPELL_AURA_MOD_IMMUNE_AURA_APPLY_SCHOOL);
        for (AuraEffectList::const_iterator iter = immuneAuraApply.begin(); iter != immuneAuraApply.end(); ++iter)
			if (/*(spellInfo->Dispel == DISPEL_MAGIC || spellInfo->Dispel == DISPEL_CURSE || spellInfo->Dispel == DISPEL_DISEASE) &&*/
...
{ 
	...
	if (sWorld->getIntConfig(CONFIG_DOUBLE_MOVING) == 1)
	{
		...
		AreaTableEntry const* zone = GetAreaEntryByAreaID(GetZoneId());
 		...
 		if (!pMap->Instanceable() && !GetUInt64Value(PLAYER_DUEL_ARBITER))
		{
...
{ 
    ...
    {
        if (m_mmapNotAcceptableStartTime) m_mmapNotAcceptableStartTime = 0;
        SetInFront(target);
        return target;
    }
...
{ 
    if (GetTypeId() == TYPEID_UNIT && GetHoverHeight() >= 2.0f && !HasUnitMovementFlag(MOVEMENTFLAG_FALLING) && !movespline->isFalling())
...
{ 
    ...
    for (uint16 index = 0; index < m_valuesCount; ++index)
    {
            ...
            (index == UNIT_FIELD_AURASTATE && HasFlag(UNIT_FIELD_AURASTATE, PER_CASTER_AURA_STATE_MASK)))
        {
            ...
            {
				if (target->IsSpectator() && target->FindMap() && target->FindMap()->IsBattleArena() && (this->GetTypeId() == TYPEID_PLAYER || this->GetTypeId() == TYPEID_UNIT || this->GetTypeId() == TYPEID_DYNAMICOBJECT))
				{
