...
{ 
    ...
    float value = GetModifierValue(unitMod, BASE_VALUE);//  Do NOT add base health TWICE + GetCreateHealth();
    value *= GetModifierValue(unitMod, BASE_PCT);
...
{ 
    ...
    if (GetEntry() == NPC_IMP)                                     // imp's attack power
		...
		val = 589 /* base ap!*/ + GetStat(STAT_STRENGTH) + GetStat(STAT_AGILITY);
	else
