...
enum RuneCooldowns
{
    ...
    RUNE_GRACE_PERIOD   = 2500,     //  maximum possible grace period
...
class Player : public Unit, public GridObject<Player>
{
    ...
    public:
        ...
        void SendMessageToSet(WorldPacket* data, bool self) { SendMessageToSetInRange(data, GetVisibilityRange(), self, true); }
        void SendMessageToSetInRange(WorldPacket* data, float dist, bool self, bool includeMargin = false, Player const* skipped_rcvr = NULL);
        void SendMessageToSetInRange_OwnTeam(WorldPacket* data, float dist, bool self);
		void SendMessageToSet(WorldPacket* data, Player const* skipped_rcvr) { SendMessageToSetInRange(data, GetVisibilityRange(), skipped_rcvr != this, true, skipped_rcvr); }
...
//        Spell* m_spellModTakingSpell;  // Spell for which charges are dropped in spell::finish
    ...
    private:
        ...
        bool MustDelayTeleport() const { return m_bMustDelayTeleport; }
...
{ 
    ...
    for (SpellModList::iterator itr = m_spellMods[op].begin(); itr != m_spellMods[op].end(); ++itr)
    {
		...
		//  temporary pets cannot use charged mods of owner, needed for mirror image QQ they should use their own auras
		if (temporaryPet && mod->charges != 0)
			continue;

        if (mod->type == SPELLMOD_FLAT)
		{
			//  do not allow to consume more than one 100% crit increasing spell
        ...
        else if (mod->type == SPELLMOD_PCT)
        {
            ...
            if (mod->op == SPELLMOD_CASTING_TIME && basevalue >= T(10000) && mod->value <= -100)
                continue;
			//  special exception for surge of light, dont affect crit chance if previous mods were not applied
			else if (mod->op == SPELLMOD_CRITICAL_CHANCE && spell && !HasSpellMod(mod, spell))
				continue;
			//  special case for backdraft gcd reduce with backlast time reduction, dont affect gcd if cast time was not applied
			else if (mod->op == SPELLMOD_GLOBAL_COOLDOWN && spell && !HasSpellMod(mod, spell))
				continue;

			//  those two mods should be multiplicative (Glyph of Renew)
