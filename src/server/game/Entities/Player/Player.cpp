...
inline void KillRewarder::_InitXP(Player* player)
{
	...
	if (_xp && !_isBattleGround && _victim)
...
{ 
    ...
    else if (m_additionalSaveTimer && !GetSession()->isLogingOut())
    {
...
{ 
	...
	itr->second->specMask = (((uint8)itr->second->specMask) & ~removeSpecMask);
    ...
    else
    {
        ...
        for (SkillLineAbilityMap::const_iterator _spell_idx = bounds.first; _spell_idx != bounds.second; ++_spell_idx)
        {
            ...
            if (_spell_idx->second->learnOnGetSkill == ABILITY_LEARNED_ON_GET_RACE_OR_CLASS_SKILL && pSkill->categoryId != SKILL_CATEGORY_CLASS ||
                (pSkill->id == SKILL_LOCKPICKING || pSkill->id == SKILL_RUNEFORGING) && _spell_idx->second->max_value == 0)
            {
...
{ 
    ...
    for (itr = m_spellCooldowns.begin(); itr != m_spellCooldowns.end(); itr = next)
    {
		...
		if (spellInfo->HasAttribute(SPELL_ATTR4_USABLE_IN_ARENA))
			...
			&& itr->second.maxduration < 10 * MINUTE * IN_MILLISECONDS)
			RemoveSpellCooldown(itr->first, true);
    }
...
{ 
    ...
    for (; range.first != range.second; ++range.first)
    {
        ...
        if (GetQuestStatus(exclude_Id) != QUEST_STATUS_NONE || (!(qInfo->IsRepeatable() && Nquest->IsRepeatable()) && !Nquest->IsSeasonal() && IsQuestRewarded(exclude_Id)))
        {
            if (msg)
                SendCanTakeQuestResponse(INVALIDREASON_DONT_HAVE_REQ);
            return false;
...
#define RelocateToHomebind(){ mapId = m_homebindMapId; instanceId = 0; Relocate(m_homebindX, m_homebindY, m_homebindZ); }
    ...
    else if (transLowGUID)
    {
		...
		if (transGO)
            if (transGO->IsInWorld() && transGO->FindMap())
    ...
    */
    ...
    if (result)
    {
        do
        {
			...
			if (remaintime != -1 && ((!spellInfo->IsPositive() && spellInfo->Id != 15007) || spellInfo->HasAttribute(SPELL_ATTR4_FADES_WHILE_LOGGED_OUT)))
            {
...
{ 
    ...
    float bestDist = SIZE_OF_GRIDS*SIZE_OF_GRIDS;
	float currDist = 0.0f;
    ...
    if (cat && catrec > 0)
    {
        ...
        if (i_scstore != sSpellsByCategoryStore.end())
        {
            for (SpellCategorySet::const_iterator i_scset = i_scstore->second.begin(); i_scset != i_scstore->second.end(); ++i_scset)
            {
				...
				AddSpellCooldown(*i_scset, itemId, catrecTime, !spellInfo->IsCooldownStartedOnEvent() && spellInfo->CategoryRecoveryTime != spellInfo->RecoveryTime && spellInfo->RecoveryTime && spellInfo->CategoryRecoveryTime);
            }
        }
    }
}
...
{ 
    ...
    if (!skipGrace)
        cooldown -= GetGracePeriod(index) < 250 ? 0 : GetGracePeriod(index)-250;
    ...
    *data << uint8(m_activeSpec);                           // talent group index (0 or 1)
    ...
    for (uint32 specIdx = 0; specIdx < m_specsCount; ++specIdx)
    {
        ...
        *data << uint8(talentIdCount);                      // [PH], talentIdCount
		...
		for (PlayerTalentMap::const_iterator itr = talentMap.begin(); itr != talentMap.end(); ++itr)
			if (TalentSpellPos const* talentPos = GetTalentSpellPos(itr->first))
				if (itr->second->State != PLAYERSPELL_REMOVED && itr->second->IsInSpec(specIdx))
				{
...
{ 
	...
	if (m_entryPointData.HasTaxiPath())
	{
		for (size_t i = 0; i < m_entryPointData.taxiPath.size(); ++i)
			ss << m_entryPointData.taxiPath[i] << ' ';
	}
