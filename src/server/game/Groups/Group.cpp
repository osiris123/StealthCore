...
bool Group::AddMember(Player* player)
{
    ...
    if (player)
    {
		...
		else if (IsLeader(player->GetGUID()) && isLFGGroup())
		{
			Player::ResetInstances(player->GetGUIDLow(), INSTANCE_RESET_GROUP_JOIN, false);
		}
...
bool Group::RemoveMember(uint64 guid, const RemoveMethod &method /*= GROUP_REMOVEMETHOD_DEFAULT*/, uint64 kicker /*= 0*/, const char* reason /*= NULL*/)
{
    ...
    if (GetMembersCount() > ((isBGGroup() || isLFGGroup() || isBFGroup()) ? 1u : 2u))
    {
        ...
        for (Rolls::iterator it = RollId.begin(); it != RollId.end();)
        {
			...
			//  itr can be erased inside
			//  player is removed from all vote lists so it will not pass above playerVote == playerVote.end statement during second iteration
...
bool Group::CountRollVote(uint64 playerGUID, uint64 Guid, uint8 Choice)
{
	...
	//  if choice == MAX_ROLL_TYPE, player was removed from the map in removefromgroup
	//  itr can be invalid as it is not used below
