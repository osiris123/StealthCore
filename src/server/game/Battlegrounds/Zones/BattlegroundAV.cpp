...
void BattlegroundAV::EventPlayerAssaultsPoint(Player* player, uint32 object)
{
	...
	//  moved here, assure that no call to m_Nodes is used in IF statement bellow as it is modified
    ...
    //  change here is too late, AssaultNode(node, team);
    UpdateNodeWorldState(node);
...
void BattlegroundAV::SendMineWorldStates(uint32 mine)
{
	...
	for (uint8 i = 0; i < 3; ++i)
		UpdateWorldState(BG_AV_MineWorldStates[mine][i], 0); //  Clear data for consistency and buglessness
