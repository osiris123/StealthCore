...
void BattlegroundMgr::Update(uint32 diff)
{
    ...
    if (!m_ArenaQueueUpdateScheduler.empty())
    {
        ...
        for (uint8 i = 0; i < scheduled.size(); i++)
        {
            ...
            m_BattlegroundQueues[bgQueueTypeId].BattlegroundQueueUpdate(bracket_id, 0x03, true, arenaRatedTeamId);
        }
    }
    ...
    if (m_NextPeriodicQueueUpdateTime < diff)
    {
        ...
        for (uint32 qtype = BATTLEGROUND_QUEUE_2v2; qtype <= BATTLEGROUND_QUEUE_5v5; ++qtype)
            for (uint32 bracket = BG_BRACKET_ID_FIRST; bracket < MAX_BATTLEGROUND_BRACKETS; ++bracket)
                m_BattlegroundQueues[qtype].BattlegroundQueueUpdate(BattlegroundBracketId(bracket), 0x03, true, 0);
...
void BattlegroundMgr::BuildPvpLogDataPacket(WorldPacket* data, Battleground* bg)
{
    ...
    *data << uint8(type);                                   // type (battleground=0/arena=1)
    ...
    if (type)                                               // arena
    {
        ...
        {
            // : oryginally this was looping in reverse order, loop order was changed so we have to change checked teamId
