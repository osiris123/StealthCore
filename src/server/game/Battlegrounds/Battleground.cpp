                ...
                if (Player* player = itr->second)
                {
                    ...
                    for (Unit::AuraApplicationMap::iterator iter = auraMap.begin(); iter != auraMap.end();)
                    {
                        ...
                        if (!aura->IsPermanent()
                            ...
                            && aurApp->IsPositive()
                            // && (!aura->GetSpellInfo()->HasAttribute(SPELL_ATTR0_UNAFFECTED_BY_INVULNERABILITY))  bullshit condition, ALL buffs should be removed
...
{
	//  if this is true, it means that endbattleground is called second time
	...
	bool bValidArena = isArena() && isRated() && GetStatus() == STATUS_IN_PROGRESS && GetStartTime() >= startDelay+15000;
    ...
    for (BattlegroundPlayerMap::iterator itr = m_Players.begin(); itr != m_Players.end(); ++itr)
    {
        ...
        if (bgTeamId == winnerTeamId)
        {
            if (player->IsCurrentBattlegroundRandom() || BattlegroundMgr::IsBGWeekend(GetBgTypeID()))
            {
                ...
                //  check player level and not bracket level if (CanAwardArenaPoints())
...
Creature* Battleground::AddCreature(uint32 entry, uint32 type, float x, float y, float z, float o, uint32 respawntime, MotionTransport* transport)
{
	...
	//  Set PVP state for vehicles, should be for all creatures in bg?
