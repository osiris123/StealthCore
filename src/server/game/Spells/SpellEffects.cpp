...
void Spell::EffectSchoolDMG(SpellEffIndex effIndex)
{
    ...
    if (unitTarget && unitTarget->IsAlive())
    {
        ...
        switch (m_spellInfo->SpellFamilyName)
        {
            ...
            case SPELLFAMILY_WARRIOR:
            {
                ...
                if (m_spellInfo->SpellFamilyFlags[1] & 0x200 && m_spellInfo->GetCategory() == 1209)
                {
					...
					// : shield block should increase the limit
        ...
        if (m_originalCaster /*&& damage > 0 : this can be increased from 0*/ && apply_direct_bonus)
        {
			// : protection
			if (damage < 0)
				damage = 0;
            ...
            case 35729:
            {
                ...
                for (Unit::AuraApplicationMap::iterator iter = Auras.begin(); iter != Auras.end();)
                {
                    ...
                    if ((spell->DmgClass == SPELL_DAMAGE_CLASS_MAGIC || (spell->GetDispelMask() & dispelMask) || dmgClassNone) &&
						...
						// : Ignore NPC spells having INVULNERABILITY attribute
...
void Spell::EffectJumpDest(SpellEffIndex effIndex)
{
	...
	// : this can happen if MovePositionToFirstCollision detects that X, Y cords are invalid and returns prematurely
...
void Spell::EffectPersistentAA(SpellEffIndex effIndex)
{
    ...
    if (!m_spellAura)
    {
        ...
        if (!caster->IsInWorld() || !caster->FindMap() || !ObjectAccessor::GetUnit(*caster, caster->GetGUID()))
            return;
...
void Spell::EffectOpenLock(SpellEffIndex effIndex)
{
    ...
    if (gameObjTarget)
    {
		...
		else if (m_spellInfo->Id == 1842 && gameObjTarget->GetGOInfo()->type == GAMEOBJECT_TYPE_TRAP)
        {
			...
			if (!gameObjTarget->GetOwner())
				gameObjTarget->SetRespawnTime(gameObjTarget->GetGOInfo()->GetAutoCloseTime()/IN_MILLISECONDS/**/);
            return;
        }
...
void Spell::EffectSummonType(SpellEffIndex effIndex)
{
    ...
    switch (properties->Category)
    {
        ...
        case SUMMON_CATEGORY_UNK:
            ...
            switch (properties->Type)
            {
                ...
                case SUMMON_TYPE_MINIPET:
                {
					...
					// : Pet can have some auras in creature_addon or in scripts, do not remove them instantly
        ...
        case SUMMON_CATEGORY_PET:
			// : SummonGuardian function can summon a few npcs of same type, remove old summons with same entry here
			if (m_originalCaster)
        ...
        case SUMMON_CATEGORY_VEHICLE:
			...
			// : vehicles summoned in air, eg. Cold Hearted quest
			...
			if (basePoints > 1) // : some summoning spells have value 1 - indicates vehicle seat
			{
			...
			// : i think this is wrong, found only 2 vehicles with faction override and one of them should inherit caster faction...
...
void Spell::EffectSummonPet(SpellEffIndex effIndex)
{
	...
	// : cant execute this... :( hope nothing relevant gets bugged
...
void Spell::EffectTaunt(SpellEffIndex /*effIndex*/)
{
	...
	// : Hand of Reckoning, cast before checing canhavethreatlist. fixes damage against pets
...
void Spell::EffectWeaponDmg(SpellEffIndex effIndex)
{
	...
	// : Divine Storm deals normalized damage
...
void Spell::EffectThreat(SpellEffIndex /*effIndex*/)
{
	...
	// : skip if target cannot have threat list or caster is friendly (ghoul leap)
...
void Spell::EffectSanctuary(SpellEffIndex /*effIndex*/)
{
	...
	if (unitTarget->GetInstanceScript() && unitTarget->GetInstanceScript()->IsEncounterInProgress())
	{
		...
		// : replaced with CombatStop(false)
...
void Spell::EffectAddComboPoints(SpellEffIndex /*effIndex*/)
{
	...
	if (m_spellInfo->Id == 14157 || // Ruthlessness and Netherblade set
		m_spellInfo->Id == 70802)   // : mayhem, rogue t10p4
    {
...
void Spell::EffectSummonPlayer(SpellEffIndex /*effIndex*/)
{
	...
	// : Unit Target may be on other map!!!, Need workaround
...
void Spell::EffectActivateObject(SpellEffIndex /*effIndex*/)
{
	...
	// : pass player to allow gossip scripts to work
	//
...
void Spell::EffectEnchantHeldItem(SpellEffIndex effIndex)
{
    ...
    if (m_spellInfo->Effects[effIndex].MiscValue)
    {
		...
		// : Venomhide poison, no other spell uses this effect...
...
void Spell::EffectForceDeselect(SpellEffIndex /*effIndex*/)
{
	...
	// : Mirror images code Initialize Images
...
void Spell::EffectKnockBack(SpellEffIndex effIndex)
{
	...
	// : allow entry specific spells to skip those checks
...
void Spell::EffectPullTowards(SpellEffIndex effIndex)
{
    ...
    {
		// : Increase Z position a little bit, should protect from falling through textures
...
void Spell::EffectResurrectPet(SpellEffIndex /*effIndex*/)
{
    ...
    pet->ClearUnitState(uint32(UNIT_STATE_ALL_STATE & ~(UNIT_STATE_POSSESSED))); // : just in case
...
void Spell::EffectActivateRune(SpellEffIndex effIndex)
{
    ...
    for (uint32 j = 0; j < MAX_RUNES && count > 0; ++j)
    {
        if (player->GetRuneCooldown(j) && player->GetCurrentRune(j) == RuneType(m_spellInfo->Effects[effIndex].MiscValue))
        {
            ...
            player->SetGracePeriod(j, player->IsInCombat()); // : reset grace period
            --count;
        }
    }
    ...
    if (m_spellInfo->Id == 45529 && count > 0)
    {
        for (uint32 l = 0; l < MAX_RUNES && count > 0; ++l)
        {
            ...
            if ((player->GetRuneCooldown(l) && player->GetCurrentRune(l) == RuneType(m_spellInfo->Effects[effIndex].MiscValueB)) && (player->GetRuneCooldown(l+1) && player->GetCurrentRune(l+1) == RuneType(m_spellInfo->Effects[effIndex].MiscValueB)))
            {
                ...
                player->SetGracePeriod(l, player->IsInCombat()); // : reset grace period
                --count;
            }
            else
                break;
    ...
    if (m_spellInfo->Id == 47568)
    {
        ...
        for (uint32 i = 0; i < MAX_RUNES; ++i)
        {
            if (player->GetRuneCooldown(i) && (player->GetCurrentRune(i) == RUNE_FROST ||  player->GetCurrentRune(i) == RUNE_DEATH))
            {
                ...
                player->SetGracePeriod(i, player->IsInCombat()); // : reset grace period
            }
        }
    }
...
void Spell::SummonGuardian(uint32 i, uint32 entry, SummonPropertiesEntry const* properties, uint32 numGuardians)
{
    ...
    for (uint32 count = 0; count < numGuardians; ++count)
    {
		...
		// : do not use precalculated position for effect summon pet in this function, it means it was cast by NPC and should have its position overridden
		...
		// : set calculated level
		summon->SetLevel(summonLevel);

		// : if we have more than one guardian, change follow angle
		...
		// : move this here, some auras are added in initstatsforlevel!
	...
	// : have to do it here because in Pet init stats infernal is not in world, imo this should be changed...
	if (summon)
	{
		switch (m_spellInfo->Id)
		{
...
void Spell::EffectSummonRaFFriend(SpellEffIndex effIndex)
{
	...
	// : Unit Target can be on other map, thank god we dont use RaF...
