...
bool SpellEffectInfo::IsFarUnitTargetEffect() const
{
    return (Effect == SPELL_EFFECT_SUMMON_PLAYER)
        ...
        /*|| (Effect == SPELL_EFFECT_SKIN_PLAYER_CORPSE) : This is not Far Unit Target Effect... what a bullshit*/;
}
...
int32 SpellEffectInfo::CalcValue(Unit const* caster, int32 const* bp, Unit const* /*target*/) const
{
	...
	// : added basePointsPerLevel check
...
bool SpellInfo::IsRangedWeaponSpell() const
{
    return (SpellFamilyName == SPELLFAMILY_HUNTER && !(SpellFamilyFlags[1] & 0x10000000)) // for 53352, cannot find better way
		...
		|| (Attributes & SPELL_ATTR0_REQ_AMMO); // : added
}
...
bool SpellInfo::CanPierceImmuneAura(SpellInfo const* aura) const
{
    ...
    if ((AttributesEx & SPELL_ATTR1_UNAFFECTED_BY_SCHOOL_IMMUNE)
        // ...but not these (Divine shield for example)																					// : banish exception, banish can override banish to cancel itself
...
{
	// : Passive auras cannot be dispelled
	if (aura->IsPassive())
		return false;

    // : At frist we check non-player auras, that should be never dispellable
    // : Eg. Mark of the Fallen Champion
...
bool SpellInfo::IsStrongerAuraActive(Unit const* caster, Unit const* target) const
{
	...
	for (uint8 i = EFFECT_0; i < MAX_SPELL_EFFECTS; ++i)
	{
		...
		// : if non-aura effect is preset - return false
		if (!Effects[i].IsAura())
			return false;

		// : aura is periodic - return false
	...
	for (uint8 i = EFFECT_0; i < MAX_SPELL_EFFECTS; ++i)
	{
		...
		{
			// : aura is not groupped or in different group
			...
			// : check aura effect equal auras only, some auras have different effects on different ranks - check rank also
			if (!IsAuraEffectEqual((*iter)->GetSpellInfo()) && !IsRankOf((*iter)->GetSpellInfo()))
				continue;

			// : misc value mismatches
			// : commented, checked above
			//if (Effects[i].MiscValue != (*iter)->GetMiscValue())
			//	continue;

			// : should not happen, or effect is not active - stronger one is present
			...
			// : take custom values into account

			int32 basePoints = Effects[i].BasePoints;
			int32 duration = GetMaxDuration();

			// : should have the same id, can be different if spell is triggered
			// : have to fix spell mods for triggered spell, turn off current spellmodtakingspell for preparing and restore after
			...
			// : for same spells, divide amount by stack amount
			if (Id == (*iter)->GetId())
			...
			// : little hack, if current spell is the same as aura spell, asume it is not stronger
			// : if values are the same, duration mods should be taken into account but they are almost always passive
			if (curValue == auraValue)
			{
				if (Id == (*iter)->GetId())
					continue;
...
bool SpellInfo::ValidateAttribute6SpellDamageMods(const Unit* caster, const AuraEffect* auraEffect, bool isDot) const
{
	...
	//  we have a hook to decide which auras should profit to the spell, by default no profits
	...
	// : Necrosis, no profits for done and taken
	else if (Id == 51460)
		return false;

	// : Do not affect dots and auras obtained from items, only affected by self casted auras
...
SpellCastResult SpellInfo::CheckTarget(Unit const* caster, WorldObject const* target, bool implicit) const
{
	...
	// : check if stronger aura is active
