...
SpellGroupStackFlags SpellMgr::CheckSpellGroupStackRules(SpellInfo const* spellInfo1, SpellInfo const* spellInfo2, bool remove, bool areaAura) const
{
	...
	if (groupId > 0 && groupId == GetSpellGroup(spellid_2))
	{
		...
		// : first flags are used for elixir stacking rules
...
bool SpellMgr::IsSpellProcEventCanTriggeredBy(SpellInfo const* spellProto, SpellProcEventEntry const* spellProcEvent, uint32 EventProcFlag, SpellInfo const* procSpell, uint32 procFlags, uint32 procExtra, bool active) const
{
    ...
    // : Always trigger for this, including TAKEN_DAMAGE
...
void SpellMgr::LoadSpellRequired()
{
    ...
    do
    {
		...
		// : fill additionalTalentInfo data, currently Blessing of Sanctuary only
...
void SpellMgr::LoadEnchantCustomAttr()
{
    ...
    for (uint32 i = 0; i < GetSpellInfoStoreSize(); ++i)
    {
		...
		// : commented second part, fixes warlock enchants like firestone, spellstone
...
void SpellMgr::LoadSpellCustomAttr()
{
    ...
    for (uint32 i = 0; i < GetSpellInfoStoreSize(); ++i)
    {
        ...
        switch (spellInfo->Id)
        {
			// : additional spells which should be binary
			...
			// : NOT CUSTOM, cant add in DBC CORRECTION because i need to swap effects, too much work to do there
			// Envenom
			case 32645:
			case 32684:
			case 57992:
...
void SpellMgr::LoadDbcDataCorrections()
{
    ...
    for (uint32 i = 0; i < sSpellStore.GetNumRows(); ++i)
    {
        ...
        for (uint8 j = 0; j < MAX_SPELL_EFFECTS; ++j)
        {
			...
			// : i hope this will fix the problem with not working resurrection
			if (spellInfo->Effect[j] == SPELL_EFFECT_SELF_RESURRECT)
				spellInfo->EffectImplicitTargetA[j] = TARGET_UNIT_CASTER;
        }

		// : Fix range for trajectories and triggered spells
		for (uint8 j = 0; j < 3; ++j)
        ...
        case 44544:
            ...
            spellInfo->EffectSpellClassMask[0] = flag96(685904631, 1151040, 32); // : removed molten armor
            break;
		...
		case 63058:
			...
			break;
                // CN Double Movement Speed for Druid Flightforms
                case 33948:
                   spellInfo->Effect[1] = 300;
                   break;
                case 40121:
                   spellInfo->Effect[i] = 560;
                   break;
		...
		case 72444: // Mark of the Fallen Champion (Deathbringer Saurfang) // : prevented in script, effect needed for Prayer of Mending
		case 72445: // Mark of the Fallen Champion (Deathbringer Saurfang)
		case 72446: // Mark of the Fallen Champion (Deathbringer Saurfang)
			spellInfo->AttributesEx3 &= ~SPELL_ATTR3_CANT_TRIGGER_PROC;
			break;
	...
	for (uint32 i = 0; i < sAreaStore.GetNumRows(); ++i)
		if (AreaTableEntry* areaEntry = const_cast<AreaTableEntry*>(sAreaStore.LookupEntry(i)))
		{
			...
			// : Dun Morogh, Kharanos tavern, missing resting flag
