        ...
        if (m_originalCaster)
        {
            ...
            if (m_spellAura)
            {
                ...
                else
                {
                    ...
                    // : apply relic cooldown, imo best place to add this
...
bool Spell::UpdateChanneledTargetList()
{
                ...
                if (channelAuraMask & ihit->effectMask)
                {
                    if (AuraApplication * aurApp = unit->GetAuraApplication(m_spellInfo->Id, m_originalCasterGUID))
                    {
                        if (m_caster != unit)
						{
							...
							// : Update Orientation server side (non players wont sent appropriate packets)
	...
	// : not all effects are covered, remove applications from all targets
...
void Spell::prepare(SpellCastTargets const* targets, AuraEffect const* triggeredByAura)
{
	...
	// : if spell have nearby target entry only, do not allow to cast if no targets are found
	if (m_CastItem)
	{
		...
		for (uint8 i = EFFECT_0; i < MAX_SPELL_EFFECTS; ++i)
		{
			...
			// : by default set it to false, and to true if any valid target is found
			selectTargets = true;
		}
    ...
    else
    {
        ...
        if ((m_casttime || m_spellInfo->IsChanneled()) && !(_triggeredCastFlags & TRIGGERED_IGNORE_SET_FACING))
		{
			...
			{
				// : Creature should focus to cast target if there is explicit target or self if casting positive spell
				// : Creature should not rotate like a retard when casting spell... based on halion behavior
				m_caster->ToCreature()->FocusTarget(this, m_targets.GetObjectTarget() != NULL ? m_targets.GetObjectTarget() : m_caster);
			}
		}

        //item: first cast may destroy item and second cast causes crash
		// : removed !m_spellInfo->StartRecoveryTime because of TC retardiness
		// second los check failed in events
		// : removed itemguid check, currently there is no such item in database
...
void Spell::cancel(bool bySelf)
{
    ...
    switch (oldState)
    {
        ...
        case SPELL_STATE_DELAYED:
            SendInterrupted(0);
            // : fixes bugged gcd reset in some cases
            if (!autoRepeat)
...
void Spell::_cast(bool skipCheck)
{
	...
	// : implement attribute SPELL_ATTR1_DISMISS_PET, on spell cast current pet is dismissed and charms are removed
    ...
    if (Player* playerCaster = m_caster->ToPlayer())
    {
        ...
        // : take into account SPELL_ATTR3_NO_INITIAL_AGGRO
    ...
    if ((m_spellInfo->Speed > 0.0f && !m_spellInfo->IsChanneled())/* : we dont need this shit || m_spellInfo->Id == 14157*/)
    {
		...
		// : Fucking retarded idiots, why do we keep focus after spell is sent to air?
		// : Because of this, in the middle of some animation after setting targetguid to 0 etc
		// : we get focused to it out of nowhere, idiots...
...
void Spell::_handle_finish_phase()
{
    ...
    if (!IsAutoRepeat() && !IsNextMeleeSwingSpell())
        if (Player* p = m_caster->GetCharmerOrOwnerPlayerOrPlayerItself())
            for (std::list<TargetInfo>::iterator ihit = m_UniqueTargetInfo.begin(); ihit != m_UniqueTargetInfo.end(); ++ihit)
            {
				// : Properly clear infinite cooldowns in some cases
...
{
	// : properly add creature cooldowns
    if (m_caster->GetTypeId() != TYPEID_PLAYER)
	{
		if (!(_triggeredCastFlags & TRIGGERED_IGNORE_SPELL_AND_CATEGORY_CD))
		{
			...
			// : this adds cooldowns to vehicle spells which misses them client-side (when we overwrote dbc info in eg.)
...
void Spell::update(uint32 difftime)
{
	...
	// : added preparing state (real cast, skip channels as they have other flags for this)
    ...
    switch (m_spellState)
    {
        ...
        case SPELL_STATE_CASTING:
        {
			...
			// : Dont update channeled target list on last tick, allow auras to update duration properly
			// : Added this strange check because of diffrent update routines for players / creatures
			// : If creature gets new aura in 800ms and in 840ms its updated with diff 270, not 40 is removed from duration but 270
			// : so the aura can be removed in different updates for all units
...
void Spell::finish(bool ok)
{
    ...
    if (!ok)
	{
		if (m_caster->GetTypeId() == TYPEID_PLAYER)
		{
			// : Restore spell mods in case of fail cast
			m_caster->ToPlayer()->RestoreSpellMods(this);

			// : Reset cooldown event in case of fail cast
	...
	// : clear reactive auras states after spell cast
...
void Spell::SendCastResult(SpellCastResult result)
{
	...
	// : override every possible result, except for gm fail result... breaks many things and goes unnoticed because of this and makes me rage when i find this out
...
void Spell::WriteSpellGoTargets(WorldPacket* data)
{
    ...
    for (std::list<TargetInfo>::const_iterator ihit = m_UniqueTargetInfo.begin(); ihit != m_UniqueTargetInfo.end() && hit < 255; ++ihit)
    {
        if ((*ihit).missCondition == SPELL_MISS_NONE)       // Add only hits
        {
            *data << uint64(ihit->targetGUID);
			// : WTF is this? No channeled spell checked, no anything
    ...
    *data << (uint8)0; // placeholder
	...
	// : Why do we reset something that is not set??????
    //if (!m_spellInfo->IsChanneled())
    //    m_channelTargetEffectMask = 0;
...
void Spell::TakeRunePower(bool didHit)
{
	...
	// : firstly consume death runes of base type
	// : in second loop consume all available
...
SpellCastResult Spell::CheckCast(bool strict)
{
	...
	// : do not check explicit casts for self cast of triggered spells (eg. reflect case)
	if (!(m_spellInfo->HasAttribute(SPELL_ATTR0_PASSIVE) && (!m_targets.GetUnitTarget() || m_targets.GetUnitTarget() == m_caster)))
    {
        // Check explicit target for m_originalCaster - todo: get rid of such workarounds
		// : do not check explicit target for triggered spell casted on self with targetflag enemy
	...
	// : do not skip triggered spells if they posses prevention type (eg. Bladestorm vs Hand of Protection)
    ...
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
    {
        ...
        switch (m_spellInfo->Effects[i].Effect)
        {
            ...
            case SPELL_EFFECT_CHARGE:
            {
                ...
                if (m_caster->GetTypeId() == TYPEID_PLAYER)
                    if (Unit* target = m_targets.GetUnitTarget())
                        if (!target->IsAlive())
                            return SPELL_FAILED_BAD_TARGETS;
				// : Pass only explicit unit target spells
            ...
            case SPELL_EFFECT_SUMMON_PLAYER:
            {
				...
				// : Implement summon pending error
			...
			// : do not allow to use leaps while rooted
    ...
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
    {
        switch (m_spellInfo->Effects[i].ApplyAuraName)
        {
            ...
            case SPELL_AURA_AOE_CHARM:
            {
				...
				// : allow SPELL_AURA_MOD_POSSESS to posses target if caster has some pet
            ...
            {
				// : disallow casting in water for mounts not increasing water movement speed
				...
				// : dont allow to cast mounts in specific transforms
...
SpellCastResult Spell::CheckPetCast(Unit* target)
{
	...
	// : Calculate power cost here, so funciton checking power can work properly and dont return shit results
...
SpellCastResult Spell::CheckCasterAuras(bool preventionOnly) const
{
	...
	// : if spell is triggered check preventionType only
	if (!preventionOnly)
	{
	...
	// : if there is no prevented_reason, check prevention types
...
SpellCastResult Spell::CheckRange(bool strict)
{
    ...
    if (target)
    {
		if (target != m_caster)
		{
			// : WHAT DA FUCK IS THIS SHIT? Spells with 5yd range can hit target 9yd away? >.>
		...
		// : check min range for self casts
...
SpellCastResult Spell::CheckItems()
{
    ...
    {
		// : this is not true in my opinion, in eg bladestorm will not be canceled after disarm
        ...
        switch (m_spellInfo->Effects[i].Effect)
        {
            ...
            case SPELL_EFFECT_CREATE_ITEM_2:
            {
                ...
                if (target && target->GetTypeId() == TYPEID_PLAYER && !IsTriggered() && m_spellInfo->Effects[i].ItemType)
                {
					...
					// : why do we check p_caster? retards p_caster->CanStoreNewItem(...)
...
bool Spell::CheckEffectTarget(Unit const* target, uint32 eff) const
{
	...
	// : skip los checking if spell has appropriate attribute, or target requires specific entry
...
void Spell::TriggerGlobalCooldown()
{
    ...
    if (!gcd)
	{
		// : fix for charmed pet spells with no cooldown info
...
bool WorldObjectSpellAreaTargetCheck::operator()(WorldObject* target)
{
	if (target->GetTypeId() == TYPEID_GAMEOBJECT)
	{
        ...
        return false;
	else if (target->GetTypeId() == TYPEID_UNIT && target->ToCreature()->IsAvoidingAOE())
		return false;
