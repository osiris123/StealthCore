...
int32 AuraEffect::CalculateAmount(Unit* caster)
{
    ...
    switch (GetAuraType())
    {
        ...
        case SPELL_AURA_TRANSFORM:
            ...
            if (!m_spellInfo->ProcFlags || m_spellInfo->HasAura(SPELL_AURA_PROC_TRIGGER_SPELL)) //  skip auras with proctriggerspell, they must have procflags...
                break;
	...
	//  save base amount, before calculating sp etc. Used for Unit::CastDelayedSpellWithPeriodicAmount
	...
	if (caster)
		switch (GetAuraType())
		{
			case SPELL_AURA_PERIODIC_DAMAGE:
				//  save caster depending auras, always pass 1 as stack amount, effect will be multiplicated at the end of the function by correct value!
...
{
	//  save caster depending auras with pct mods
...
void AuraEffect::HandleEffect(AuraApplication * aurApp, uint8 mode, bool apply)
{
	...
	//  stacking system, force return if effect is disabled, prevents double apply / unapply
	//  placed here so above line can unregister effect from the list
	//  this condition works under assumption that effect handlers performs ALL necessery action with CHANGE_AMOUNT mode
	//  as far as i've checked, all grouped spells meet this condition
...
void AuraEffect::HandleShapeshiftBoosts(Unit* target, bool apply) const
{
    ...
    if (apply)
    {
        ...
        if (player)
        {
			...
			//  talent stance auras are not on m_spells map, so iterate talents
            ...
            for (PlayerTalentMap::const_iterator itr = tl_list.begin(); itr != tl_list.end(); ++itr)
            {
				...
				//  skip talents with effect learn spell
                SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(itr->first);
    ...
    else
    {
        ...
        {           
			...
			//  Remove all passive auras, they will be added if needed
...
void AuraEffect::HandleSpiritOfRedemption(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
	...
	//  damage immunity spell, not needed because of 93 aura (adds non_attackable state)
	//  probably blizzard added it just in case in wotlk (id > 46000)
	if (apply)
...
void AuraEffect::HandleAuraModShapeshift(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (apply)
    {
		...
		//  rogue shouldnt be wrapped by this check (shadow dance vs stealth)
		...
		//  allow shapeshift to override model id if forced transform aura is not present!
        if (modelid > 0)
        {
            bool allow = true;
...
void AuraEffect::HandleModUnattackable(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (apply && (mode & AURA_EFFECT_HANDLE_REAL))
    {
		//  this aura should not stop combat (movie with sindragosa proves that)
        //target->CombatStop();
        target->RemoveAurasWithInterruptFlags(AURA_INTERRUPT_FLAG_IMMUNE_OR_LOST_SELECTION);
    }
}
...
void AuraEffect::HandleModMechanicImmunity(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    switch (GetId())
    {
		...
		case 46227: // Medalion of Immunity
            mechanic = IMMUNE_TO_MOVEMENT_IMPAIRMENT_AND_LOSS_CONTROL_MASK;
			target->RemoveAurasByType(SPELL_AURA_PREVENTS_FLEEING); //  Patch 2.3.0 PvP Trinkets: Insignia of the Alliance, Insignia of the Horde, Medallion of the Alliance, and Medallion of the Horde now clear the debuff from Judgement of Justice.
...
void AuraEffect::HandleModTotalPercentStat(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
	...
	if (GetId() == 67480) //  hack fix for blessing of sanctuary stats stack with blessing of kings...
	{
...
void AuraEffect::HandleAuraModIncreaseHealthPercent(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
	...
	//  idiots, pct was rounded down and could "kill" creature by setting its health to 0 making npc zombie
    if (target->IsAlive())
...
void AuraEffect::HandlePeriodicTriggerSpellAuraTick(Unit* target, Unit* caster) const
{
    ...
    if (triggeredSpellInfo)
    {
        if (Unit* triggerCaster = triggeredSpellInfo->NeedsToBeTriggeredByCaster(m_spellInfo, GetEffIndex()) ? caster : target)
        {
			...
			//  do not skip reagent cost for entry casts
...
void AuraEffect::HandlePeriodicHealAurasTick(Unit* target, Unit* caster) const
{
	...
	//  caster is available, checked earlier
	...
	// ignore item heals
	if (!haveCastItem && GetAuraType() != SPELL_AURA_OBS_MOD_HEALTH) //  dont allow obs_mod_health to proc spells, this is passive regeneration and not hot
		//  allow null caster to proc
		caster->ProcDamageAndSpell(target, caster ? procAttacker : 0, procVictim, procEx, damage, BASE_ATTACK, GetSpellInfo());
}
