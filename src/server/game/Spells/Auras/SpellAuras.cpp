	...
	if (uint32 groupId = aurEff->GetAuraGroup())
	{
		...
		if (!aurEff->IsPeriodic() && (sFlag & SPELL_GROUP_STACK_FLAG_EFFECT_EXCLUSIVE))
		{
			...
			for (Unit::AuraEffectList::const_iterator iter = auraList.begin(); iter != auraList.end(); ++iter)
			{
				...
				{
					//  if we have strongest aura and it is active, turn it off
					//  otherwise just save new aura;
				...
				//  itered aura is weaker, deactivate if active
			...
			//  if we have new strongest aura, and it is not active
			if (strongestApp && strongestEff && !strongestApp->IsActive(strongestEff->GetEffIndex()))
			{
...
void Aura::UpdateTargetMap(Unit* caster, bool apply)
{
    ...
    for (ApplicationMap::iterator appIter = m_applications.begin(); appIter != m_applications.end();++appIter)
    {
        ...
        {
			//  check immunities here, so aura wont get removed on every tick and then reapplied
			if (IsArea())
...
void Aura::RefreshTimersWithMods()
{
	...
	//  we should take ModSpellDuration into account, but none of the spells using this function is affected by contents of ModSpellDuration
	RefreshDuration();
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
        if (HasEffect(i))
...
bool Aura::CanBeSaved() const
{
	...
	//  Dont save control vehicle auras - caster may not exist
...
bool Aura::IsAuraStronger(Aura const* newAura) const
{
	for (uint8 i = EFFECT_0; i < MAX_SPELL_EFFECTS; ++i)
	{
		...
		for (uint8 j = EFFECT_0; j < MAX_SPELL_EFFECTS; ++j)
		{
			...
			//  assume that all spells are either positive or negative, otherwise they should not be in one group
...
bool Aura::CanStackWith(Aura const* existingAura, bool remove) const
{
	...
	//  dont allow different ranks (or the same rank) of same spell to stack (many flamestrikes for example)
    if (existingAura->GetType() == DYNOBJ_AURA_TYPE)
	{
		//  desecration lag fix - bound this condition to periodics
    ...
    if (stackFlags)
    {
		...
		//  normal exclusive stacking, remove if auras are equal by effects
		...
		//  check priority before effect mask
    ...
    if (VehicleAura1 && VehicleAura2)
    {
		...
		//  allow direct auras to stack if there is no passenger in this slot
...
void DynObjAura::FillTargetMap(std::map<Unit*, uint8> & targets, Unit* /*caster*/)
{
    ...
    for (uint8 effIndex = 0; effIndex < MAX_SPELL_EFFECTS; ++effIndex)
    {
        ...
        for (UnitList::iterator itr = targetList.begin(); itr!= targetList.end();++itr)
        {
			//  check z level and los dependence
			Unit* target = *itr;
