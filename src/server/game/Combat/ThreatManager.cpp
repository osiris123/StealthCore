...
{
	//  Do not allow to modify threat by percent if threat is negative (forced to big value < 0 by spells adding temporary threat)
	//  When the temporary effect ends, temporary threat is added back which results in huge additional amount of threat
	if (iThreat <= 0)
		return;
...
HostileReference* ThreatContainer::selectNextVictim(Creature* attacker, HostileReference* currentVictim) const
{
	...
	if (currentVictim)
	{
		...
		if (!attacker->_CanDetectFeignDeathOf(cvUnit) || !attacker->CanCreatureAttack(cvUnit) || attacker->isTargetNotAcceptableByMMaps(cvUnit->GetGUID(), currTime, cvUnit))
			currentVictim = NULL;
		else if (cvUnit->IsImmunedToDamageOrSchool(attacker->GetMeleeDamageSchoolMask()) || cvUnit->HasNegativeAuraWithInterruptFlag(AURA_INTERRUPT_FLAG_TAKE_DAMAGE))
			currentVictim = NULL;
	}
	...
	for (ThreatContainer::StorageType::const_iterator iter = iThreatList.begin(); iter != iThreatList.end() && !found;)
	{
		...
		if (attacker->_CanDetectFeignDeathOf(target) && attacker->CanCreatureAttack(target) && !attacker->isTargetNotAcceptableByMMaps(target->GetGUID(), currTime, target))
		{
			if (currentVictim)
			{
				...
				else if (currentRef->getThreat() > 1.1f * currentVictim->getThreat())
				{
