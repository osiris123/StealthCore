...
enum MovementGeneratorType
{
    ...
    ESCORT_MOTION_TYPE    = 17,                             //  EscortMovementGenerator.h
    NULL_MOTION_TYPE      = 18
};
...
class MotionMaster //: private std::stack<MovementGenerator *>
{
    ...
    public:
        ...
        {
			//  cannot be used during motion update!
            if (!(_cleanFlag & MMCF_UPDATE))
        ...
        void MoveLand(uint32 id, float x, float y, float z, float speed);
        void MoveTakeoff(uint32 id, Position const& pos, float speed);
        void MoveTakeoff(uint32 id, float x, float y, float z, float speed);
		...
		uint32 GetCurrentSplineId() const; //  Escort system
