...
void TargetedMovementGeneratorMedium<T,D>::_setTargetLocation(T* owner, bool initial)
{
    ...
    bool forceDest = (owner->FindMap() && owner->FindMap()->IsDungeon() && !isPlayerPet) || // force in instances to prevent exploiting
                     ...
                     (owner->GetMapId() == 572 && (owner->GetPositionX() < 1275.0f || i_target->GetPositionX() < 1275.0f)) ||
    ...
    else
    {
		...
		//  Fix follow angle for hostile units
		float angle = i_angle;
...
bool TargetedMovementGeneratorMedium<T,D>::DoUpdate(T* owner, uint32 time_diff)
{
    ...
    if (owner->HasUnitState(UNIT_STATE_CASTING))
    {
		...
		if (stop)
		{
			//  delay distance recheck in case of succeeding casts
    ...
    if (i_recheckDistance.Passed())
    {
		...
		//  if offset is negative (follow distance is smaller than just object sizes), reduce minimum allowed distance which is based purely on object sizes
		if (i_offset < 0.0f)
		{
