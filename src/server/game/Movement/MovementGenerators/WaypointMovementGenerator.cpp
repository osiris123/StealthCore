...
bool WaypointMovementGenerator<Creature>::StartMove(Creature* creature)
{
    ...
    if (m_isArrivalDone)
    {
		//  not true... update this at every waypoint!
	...
	//  do not initialize motion if we got stunned in movementinform
...
bool WaypointMovementGenerator<Creature>::DoUpdate(Creature* creature, uint32 diff)
{
    ...
    else
    {
        ...
        {
			//  code to detect pre-empetively if we should start movement to next waypoint
			//  do not start pre-empetive movement if current node has delay or we are ending waypoint movement
...
void FlightPathMovementGenerator::DoFinalize(Player* player)
{
	...
	//  this should be cleaned by CleanupAfterTaxiFlight(); function!
    player->Dismount();
...
bool FlightPathMovementGenerator::DoUpdate(Player* player, uint32 /*diff*/)
{
    ...
    if (pointId > i_currentNode)
    {
        ...
        do
        {
			...
			//  map should be switched, do not rely on client packets QQ
