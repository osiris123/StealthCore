...
void WorldSession::LogoutPlayer(bool save)
{
    ...
    if (_player)
    {
		...
		SavingSystemMgr::InsertToSavingSkipListIfNeeded(_player->GetNextSave());
        if (save)
        {
            uint32 eslot;
...
void WorldSession::ReadMovementInfo(WorldPacket &data, MovementInfo* mi)
{
    ...
    */
    ...
    REMOVE_VIOLATING_FLAGS(mi->HasMovementFlag(MOVEMENTFLAG_HOVER) && !GetPlayer()->m_mover->HasAuraType(SPELL_AURA_HOVER),
        MOVEMENTFLAG_HOVER);
    ...
    REMOVE_VIOLATING_FLAGS(mi->HasMovementFlag(MOVEMENTFLAG_WATERWALKING) && !GetPlayer()->m_mover->HasAuraType(SPELL_AURA_WATER_WALK),
        MOVEMENTFLAG_WATERWALKING);

    //! Cannot feather fall without SPELL_AURA_FEATHER_FALL
    REMOVE_VIOLATING_FLAGS(mi->HasMovementFlag(MOVEMENTFLAG_FALLING_SLOW) && !GetPlayer()->m_mover->HasAuraType(SPELL_AURA_FEATHER_FALL),
    ...
    */
	...
	// : Spline enabled flag should be never sent by client, its internal movementflag
