...
void World::AddSession_(WorldSession* s)
{
    ...
    if (old != m_sessions.end())
    {
		...
		{
			oldSession->SetShouldSetOfflineInDB(false);
			delete oldSession;
		}
    }
...
void World::LoadConfigSettings(bool reload)
{
    ...
    SetMotd(sConfigMgr->GetStringDefault("Motd", "Willkommen auf dem StealthCore Server."));
	...
	m_int_configs[CONFIG_TELEPORT_TIMEOUT_NEAR] = sConfigMgr->GetIntDefault("TeleportTimeoutNear", 25);
	m_int_configs[CONFIG_TELEPORT_TIMEOUT_FAR] = sConfigMgr->GetIntDefault("TeleportTimeoutFar", 45);
	m_int_configs[CONFIG_MAX_ALLOWED_MMR_DROP] = sConfigMgr->GetIntDefault("MaxAllowedMMRDrop", 500);
	m_bool_configs[CONFIG_ENABLE_LOGIN_AFTER_DC] = sConfigMgr->GetBoolDefault("EnableLoginAfterDC", true);
	m_bool_configs[CONFIG_DONT_CACHE_RANDOM_MOVEMENT_PATHS] = sConfigMgr->GetBoolDefault("DontCacheRandomMovementPaths", true);
    ...
    for (SessionMap::iterator itr = m_sessions.begin(), next; itr != m_sessions.end(); itr = next)
    {
        ...
        if (!pSession->Update(diff, updater))
        {
            if (!RemoveQueuedPlayer(pSession) && getIntConfig(CONFIG_INTERVAL_DISCONNECT_TOLERANCE))
                m_disconnects[pSession->GetAccountId()] = time(NULL);
            m_sessions.erase(itr);
			if (m_offlineSessions.find(pSession->GetAccountId()) != m_offlineSessions.end())
